...
**Setup:** 4-class; `SUBSET=False` (full known train, ~148k); cosine LR schedule, start \(0.01\); batch 64; $\lambda_1=0.5$, $\lambda_2=0.1$; same FROZEN features / circuit recipe as `4class-all-lr0.01auto-lambda2-maqt`.
...
![4class-all-lr0.01auto full known test, no reject](../img/bot-iot/4class-all-lr0.01auto-full-no-rejection-test.png)
...
previous latest: 4c / 3k / cosine start 0.01 (~65% CE full-test vs arjun ~64%)
current latest: 4c / full train / cosine start 0.01
  $\rightarrow$ ~68% CE full-test (no reject); Normal/Recon strong; DDoS↔DoS still open
```
