...
def qsnet_infer_batch_per_class(X, theta, prototypes, q_by_class, forward_circuit, p=DEFAULT_NOISE_RATE,
    ...
    label + certified radius), except the conformal test compares s(x) against
    q_by_class[c_star] -- the threshold CALIBRATED FOR c_star specifically (see
    scripts.conformal.class_conditional_calibrate) -- instead of one shared global q.

    This function is the non-cached (fresh circuit forward pass) counterpart to
    scripts.cache.cached_qsnet_infer_per_class -- use the cached version when you
    already have a ForwardCache entry for X.
    """
    if L_phi is None:
        raise ValueError("L_phi must be provided (estimate with estimate_lipschitz)")
