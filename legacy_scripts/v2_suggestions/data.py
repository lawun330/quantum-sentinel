...
def balanced_sample(X, y, n_per_class=None, seed=42):
    ...
    Kept for quick, strictly-balanced baseline runs -- but prefer
    capped_sample + class_weights_for_sampler (below) for real training,
    since undersampling throws away majority-class data.
    """
...
def capped_sample(X, y, per_class_cap, seed=42):
    """
     cap each class at `per_class_cap` samples
    (no replacement) WITHOUT forcing all classes down to the smallest one.
    Pair with class_weights_for_sampler + torch WeightedRandomSampler so the
    training loop still sees roughly-balanced batches without discarding as
    much data as balanced_sample().
    """
    rng = np.random.default_rng(seed)
    y = np.asarray(y).astype(int)
    idxs = []
...
def class_weights_for_sampler(y, n_classes):
    """
     per-sample inverse-frequency weights for torch's
    WeightedRandomSampler(weights, num_samples=len(y), replacement=True).
    """
    y = np.asarray(y).astype(int)
