...
def eval_attacked(
    ...
    **attack_kwargs,
    ...
    return {
        ...
        "macro_f1": float(
            f1_score(y_true, preds, average="macro", labels=labels, zero_division=0)
        ),
        "y_true": y_true,
        "y_pred": preds,
    }
...
def train_plain_baseline(
    ...
    theta = (
        initialize_weights_random_identity(
            n_layers=None, n_wires=n_qubits, device=device, eps=1e-2
        )
        if False
        else None
    )  # placeholder guard; caller should pass an initialized theta pattern
