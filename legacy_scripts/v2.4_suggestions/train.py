...
def _run_with_oom_retry(xb, yb, step_fn, device, max_retries):
    ...
    try:
        ...
        if _is_oom_error(e) and max_retries > 0 and len(xb) > 1:
            _free_memory(device)
            mid = len(xb) // 2
            r1 = _run_with_oom_retry(
                xb[:mid], yb[:mid], step_fn, device, max_retries - 1
            )
            r2 = _run_with_oom_retry(
                xb[mid:], yb[mid:], step_fn, device, max_retries - 1
            )
            w1, w2 = r1["n"], r2["n"]
            merged = {"n": w1 + w2}
...
def _build_payload(
    *,
    ...
    payload = {
        ...
        "step_in_epoch": int(
            step_in_epoch
        ),  # batches already done in that epoch (0 = fresh epoch)
    ...
    with _graceful_signals():
        try:
            for epoch in range(start_epoch, epochs):
                ...
                if early_stopping:
                    improved, stop = early_stop.step(
                        history["loss"][-1], epoch=epoch_1based
                    )
                    if improved:
                        best_bundle = {
                ...
                if verbose and (epoch_1based % log_every == 0):
                    ...
                    mem_msg = (
                        f" | peak_mem={history['gpu_mem_mb'][-1]:.0f}MB"
                        if history["gpu_mem_mb"][-1]
                        else ""
                    )
                    print(
                ...
                if early_stopping and stop:
                    if verbose:
                        print(
                            f"early stopping at epoch {epoch_1based}: no train-loss improvement for "
        ...
        try:
            ...
            if verbose:
                print(
                    f"warning: failed to compute final exact prototypes ({e!r}); falling back to EMA prototypes"
                )
            final_prototypes = {
                k: v.detach().clone() for k, v in ema_protos.protos.items()
            }
        ...
        try:
            for epoch in range(start_epoch, epochs):
                ...
                if verbose:
                    ...
                    print(
                        f"[plain VQC] epoch {epoch_1based}/{epochs} loss {history['loss'][-1]:.4f} "
